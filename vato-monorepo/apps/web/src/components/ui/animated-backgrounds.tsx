...
// Research Intelligence - Technical precision on warm, organic foundation
export const DarkBackground: React.FC = () => {
  // Subtle circuit board elements for warm background
  const circuitElements = Array.from({ length: 8 }, (_, i) => ({
    id: i,
    size: 200 + i * 100,
    opacity: 0.06 - i * 0.005, // Much more subtle for warm background
  ...
  const energyCores = Array.from({ length: 6 }, (_, i) => ({
    ...
    color: i % 3 === 0 ? 'rgba(96, 165, 250, 0.15)' : // Much more subtle
           i % 3 === 1 ? 'rgba(34, 211, 238, 0.12)' : 
           'rgba(167, 139, 250, 0.1)',
  }))

  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none">
      {/* Research foundation - warm beige base with subtle technical accents */}
      <div 
        className="absolute inset-0"
        style={{
          background: `
            radial-gradient(ellipse 60% 40% at 25% 20%, rgba(96, 165, 250, 0.03) 0%, transparent 70%),
            radial-gradient(ellipse 70% 50% at 80% 80%, rgba(34, 211, 238, 0.025) 0%, transparent 65%),
            radial-gradient(ellipse 50% 70% at 50% 50%, rgba(167, 139, 250, 0.02) 0%, transparent 80%)
          `
        }}
      />
      ...
      <div className="absolute inset-0">
        {dataStreams.map((stream, i) => (
          <motion.div
            ...
            animate={{
              opacity: [0, 0.15, 0.25, 0.1, 0], // Much more subtle opacity
      ...
      {energyCores.map((core) => (
        <motion.div
          ...
          animate={{
            ...
            opacity: [0, 0.3, 0.2, 0.25, 0], // Much more subtle opacity
      ...
      <motion.div
        ...
        style={{ opacity: 0.04 }} // Much more subtle
        animate={{
          left: ['-4px', '100%', '-4px'],
        }}
        transition={{
      ...
      {/* Subtle digital noise pattern */}
      <motion.div
        className="absolute inset-0"
        style={{ opacity: 0.02 }} // Much more subtle
        animate={{
        ...
        style={{
          backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' xmlns='http://www.w3.org/2000/svg'%3E%3Cdefs%3E%3Cpattern id='tech-noise' x='0' y='0' width='6' height='6' patternUnits='userSpaceOnUse'%3E%3Crect x='0' y='0' width='1' height='1' fill='%2360a5fa' opacity='0.05'/%3E%3Crect x='3' y='2' width='1' height='1' fill='%2322d3ee' opacity='0.03'/%3E%3Crect x='1' y='4' width='1' height='1' fill='%23a78bfa' opacity='0.04'/%3E%3C/pattern%3E%3C/defs%3E%3Crect width='100%25' height='100%25' fill='url(%23tech-noise)'/%3E%3C/svg%3E")`,
          opacity: 0.02, // Override with much more subtle opacity
        }}
      />
    </div>
  )
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        {themeId === 'dark' && (
          <div 
            ...
            style={{
              background: `
                radial-gradient(ellipse at 20% 10%, rgba(96, 165, 250, 0.02) 0%, transparent 50%),
                radial-gradient(ellipse at 80% 90%, rgba(34, 211, 238, 0.015) 0%, transparent 50%)
              `
            }}
          />
        )}
